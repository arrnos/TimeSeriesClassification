...
# 时间序列统计 feature构造
def feature_extraction(X_df_list, y, type='time_freq_wave'):
    """
...
def time_domain_feature(X_df_list):
    ...
    # 2. 相关系数columns
    corr_columns = ["corr_" + str(x) for x in range(6)]  # len(corr_fea)
    cov_columns = ["cov_" + str(x) for x in range(10)]  # len(cov_fea)
...
if __name__ == '__main__':
    ...
    # freq_domain_feature()
    feature_extraction(X_df_list, y)
